...
void SHT1x_Reset( void ) 
{
    ...
    for (i=9; i>0; i--)
    {
        ...
        //bcm2835_delay(GPIO_DELAY);
        ////printf("First loop delay --> %d\n", i);
        bcm2835_delay(GPIO_DELAY);
        // Set SCK to LOW
        bcm2835_gpio_write(RPI_GPIO_SHT1x_SCK, LOW);
        //bcm2835_delay(GPIO_DELAY);
        ////printf("Second loop delay-->%d\n",i);
        bcm2835_delay(GPIO_DELAY);
...
void SHT1x_Transmission_Start( void ) 
{
    ...
    //printf("Leaving Transmission start\n");
}
...
unsigned char SHT1x_Sendbyte( unsigned char value ) 
{
    ...
    for(mask = 0x80; mask; mask >>=1)
    {
        ...
        //Set SCK to HIGH
        bcm2835_gpio_write(RPI_GPIO_SHT1x_SCK, HIGH);
        bcm2835_delay(GPIO_DELAY);  // SCK hi => sensor reads data
    }
...
unsigned char SHT1x_Get_Measure_Value( unsigned short int * value ) 
{
    ...
    //printf("Enter while get measure value\n");
    while( bcm2835_gpio_lev(RPI_GPIO_SHT1x_DATA) )
    {
        bcm2835_delay(5000);            // $$$$$$$$$$$$$$$$$$ 1 ms not working $$$$$$$$$$$$$$$$$$$$$$$$
    ...
    *chPtr = SHT1x_Readbyte( 1 );        // read lo byte
    ...
    // compare it.
    //printf("Leaving get measure value\n");
...
unsigned char SHT1x_Readbyte( unsigned char send_ack ) 
{
    ...
    //Set SCK to HIGH
    bcm2835_gpio_write(RPI_GPIO_SHT1x_SCK, HIGH);
    bcm2835_delay(GPIO_DELAY);  // give a clock pulse
...
void getTempAndHumidity( tempAndHumi_t *tempAndHumiVals )
{
  ...
  tm = *localtime(&t);
	...
	//printf("Calling measure start low\n");
  ...
  //printf("Calling Get Measure Value\n");
  ...
  //printf("Calling Measure start high");
