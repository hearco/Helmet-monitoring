...
void *BodyTemp_Sensor(void *p) {
  //printf("Entering body temperature sensor\n");
  bodyTemperature = 0;
 	while(1) {
	// Only one sensor can be using the I2C pins at a time
	pthread_mutex_lock( &count_mutex );
  //printf("Calling get body temp\n");
...
void *TempAndHumidity_Sensor(void *p) {
  ...
  //printf("Entering temp and humi thread\n");
	while(1) {
	pthread_mutex_lock( &count_mutex );
	
	//printf("Calling temp and humi function\n");
  getTempAndHumidity(&tempAndHumiVals);
  humidity = tempAndHumiVals.humidity;
  envTemperature = tempAndHumiVals.envTemp;
  printf("Humidity: %f  Env Temperature: %f\n", humidity, envTemperature);
  pthread_mutex_unlock( &count_mutex );
  bcm2835_delay(2000);		
	}
}
